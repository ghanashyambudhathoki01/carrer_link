    ...
    <div class="flex flex-col md:flex-row gap-8">
        ...
        <div class="flex-1">
            <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800 font-display">All Applicants</h1>
                    <p class="text-sm font-medium text-gray-500 mt-1">Manage all applications received across your job listings.</p>
                </div>
                {{-- Status Filter --}}
                <div class="flex items-center gap-2">
                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Filter Status:</span>
                    <form action="{{ route('employer.applicants.all') }}" method="GET" id="filterForm">
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-100">
                                ...
                                <th class="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Applied For</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse($applications as $app)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-5">
                                        <a href="{{ route('jobs.show', $app->job->id) }}" class="text-xs font-bold text-blue-600 hover:underline">
                                            {{ $app->job->title }}
                                        </a>
                                        <p class="text-[10px] text-gray-400 font-medium">{{ $app->created_at->format('M d, Y') }}</p>
                                    </td>
                                    <td class="px-6 py-5">
                                            ...
                                            <div class="absolute z-20 left-0 top-full mt-1 w-32 bg-white rounded-xl shadow-xl border border-gray-100 hidden group-hover:block overflow-hidden">
                                                @foreach(['applied', 'reviewed', 'shortlisted', 'interviewed', 'rejected', 'hired'] as $status)
                                                    <form action="{{ route('employer.applicants.update', $app->id) }}" method="POST">
                                                        @csrf
                                                        @method('POST') {{-- The route might be post/put depending on implementation, web.php shows post --}}
                                    ...
                                    <td class="px-6 py-5">
                                        @if($app->resume_path)
                                            <a href="{{ asset('storage/' . $app->resume_path) }}" target="_blank" class="inline-flex items-center gap-2 px-3 py-1.5 bg-gray-50 text-gray-600 rounded-xl text-[10px] font-bold border border-gray-200 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-100 transition-all" title="Download CV">
                                    ...
                                    <td class="px-6 py-5 text-right flex items-center justify-end gap-1">
                                        <a href="mailto:{{ $app->applicant->email }}?subject=Regarding your application for {{ $app->job->title }}" class="p-2 text-gray-400 hover:text-blue-500 rounded-lg transition-all" title="Contact Applicant">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-20 text-center">
                                        ...
                                        <p class="text-sm text-gray-400 mt-1">You haven't received any applications for your jobs yet.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
